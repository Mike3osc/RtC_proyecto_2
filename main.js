...
//Coloco un Select a vendor como primera opcion creando una funcion de generacion de vendors

const genericVendor = () => {
  const allVendors = document.createElement("option");
  allVendors.textContent = "Select a vendor";
  vendorSelector.appendChild(allVendors);
...
vendorSelector.addEventListener("change", (event) => {
  ...
  let result = [];
  if (selectedVendor === "Select a vendor") {
    result = products;
  } else {
    result = products.filter((product) => product.seller === selectedVendor);
  }
  createList(result);
});
...
searchByPriceButton.addEventListener("click", (event) => {
  ...
  let result = [];
  if (selectedVendor === "Select a vendor") {
    result = products.filter((product) => budgetFiltered >= product.price);
  } else {
    result = products.filter((product) => product.seller === selectedVendor && budgetFiltered >= product.price);
  }
  if (0 === result.length) {
    alert("SHOW ME THE MONEY: Increase your budget")
  } else {
    createList(result);
  }
})

//Tercero el boton para limpiar todos los filtros y su funcionalidad
...
buttonToClean.addEventListener("click", (cleanFilter) => {
  ...
  vendorSelector.options[0].selected = true;
  createList(products);
});
